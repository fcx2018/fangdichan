...
(function (factory) {
    ...
    tProgress = '<div class="progress">\n' +
        ...
        '     </div>\n' +
        '</div>';
    tSize = ' <br><input type="hidden" value="{sizeText}" class="fileType">';
    tFooter = '<div class="file-thumbnail-footer">\n' +
        '<div class="file-footer-caption" title="{caption}"><input type="text" value="{caption}" style="border:none;"/>{size}</div>\n' +
        '    {progress} {actions}\n' +
        '</div>';
    ...
    tText = '<textarea class="kv-preview-data file-preview-text" title="{caption}" readonly ' + STYLE_SETTING +
        '>{data}</textarea>\n';
    tVideo = '<video  class="kv-preview-data" src="{data}" width="{width}" height="{height}" controls>\n' +
        '\n' + DEFAULT_PREVIEW + '\n</video>\n';
//    tVideo = '<video class="kv-preview-data" width="{width}" height="{height}" controls>\n' +
//    '<source src="{data}" type="{type}">\n' + DEFAULT_PREVIEW + '\n</video>\n';
    
    tAudio = '<audio src="{data}" class="kv-preview-data" controls>\n'+'\n' + DEFAULT_PREVIEW + '\n</audio>\n';
...
//        DEFAULT_PREVIEW + '\n</audio>\n';
    ...
    FileInput.prototype = {
        ...
        _getSize: function (bytes) {
            var self = this, size = ""+bytes;
            //size = parseFloat(bytes);
            if (!bytes || !size || isNaN(bytes) || isNaN(size)) {
                return self._getLayoutTemplate('size').replace('{sizeText}',size);//$(".btn-mark").filter(".active").attr("data-value")
            }
...
//              out = (size / Math.pow(1024, i)).toFixed(2) * 1 ;//+ ' ' + sizes[i]
            ...
            return self._getLayoutTemplate('size').replace('{sizeText}',size);//out   $(".btn-mark").filter(".active").attr("data-value")
        },
        ...
        _previewDefault: function (file, previewId, isDisabled) {
            ...
            content = self._generatePreviewTemplate('other', data, fname, ftype, previewId, isError,$(".btn-mark").filter(".active").attr("data-value"));// file.size
        ...
        _previewFile: function (i, file, theFile, previewId, data) {
            ...
            if (chkTypes || chkMimes) {
                content = self._generatePreviewTemplate(cat, iData, fname, file.type, previewId, false,$(".btn-mark").filter(".active").attr("data-value"));//file.size
                self._clearDefaultPreview();
                $previewLive.append("\n" + content);
    ...
    $.fn.fileinputLocales.en = {
            ...
            msgImageWidthSmall: '图像文件"{name}"的宽度必须至少{size}像素.',
