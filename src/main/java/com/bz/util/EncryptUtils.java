...
public class EncryptUtils {
     ...
     * @return String 加密后的字符串
     */
    public static String encryptToSHA(String info,String salt) {
    	String message=info+salt;
        byte[] digesta = null;
        try {
            // 得到一个SHA-1的消息摘要
            MessageDigest alga = MessageDigest.getInstance("SHA-1");
    ...
    public static void main(String[] args) {
        com.bz.util.EncryptUtils encryptUtils = new com.bz.util.EncryptUtils();
        String source = "cyd";
        System.out.println("Hello经过MD5:" + com.bz.util.EncryptUtils.encryptToMD5(source));
        System.out.println("Hello经过SHA:" + com.bz.util.EncryptUtils.encryptToSHA(source));
        System.out.println("Hello经过SHA:" + com.bz.util.EncryptUtils.encryptToSHA(source,"salt"));
        System.out.println("========随机生成Key进行加解密==============");
        // 生成一个DES算法的密匙
        SecretKey key = com.bz.util.EncryptUtils.createSecretDESKey();
        String str1 = com.bz.util.EncryptUtils.encryptToDES(key, source);
        ...
        // 生成一个AES算法的密匙
        SecretKey key1 = com.bz.util.EncryptUtils.createSecretAESKey();
        String stra = com.bz.util.EncryptUtils.encryptToAES(key1, source);
        System.out.println("AES加密后为:" + stra);
        // 使用这个密匙解密
        String strb = com.bz.util.EncryptUtils.decryptByAES(key1, stra);
        System.out.println("AES解密后为：" + strb);
        System.out.println("========指定Key进行加解密==============");
        try {
            String AESKey = com.bz.util.EncryptUtils.getAESKey(com.bz.util.EncryptUtils.encryptToSHA(source));
            String DESKey = com.bz.util.EncryptUtils.getDESKey(com.bz.util.EncryptUtils.encryptToSHA(source));
            System.out.println(AESKey);
            System.out.println(DESKey);
            String str11 = com.bz.util.EncryptUtils.encryptToDES(DESKey, source);
            System.out.println("DES加密后为:" + str11);
            // 使用这个密匙解密
            String str12 = com.bz.util.EncryptUtils.decryptByDES(DESKey, str11);
            System.out.println("DES解密后为：" + str12);

            // 生成一个AES算法的密匙
            String strc = com.bz.util.EncryptUtils.encryptToAES(AESKey, source);
            System.out.println("AES加密后为:" + strc);
            // 使用这个密匙解密
            String strd = com.bz.util.EncryptUtils.decryptByAES(AESKey, strc);
